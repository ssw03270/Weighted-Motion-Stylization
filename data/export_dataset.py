...
def preprocess(filename, downsample=2, slice=True, window=64, window_step=32):
    ...
    if not slice:
        ...
        cls = np.array([contents.index(cls_name.split('_')[0].split()[-1]),
                        styles.index(cls_name.split('_')[1].replace('.bvh', ''))])
        ...
        if not (cls[0] < 0) & (cls[1] < 0):
            for j in range(0, len(clip) - window // 8, window_step):
                ...
                if len(clip_slice) < window:
                    # left slices
                    clip_left = clip_slice[:1].repeat((window - len(clip_slice)) // 2 + (window - len(clip_slice)) % 2,
                                                      axis=0)
                    clip_left[:, :, -4:] = 0.0
                    clip_feet_l = clip_feet[:1].repeat((window - len(clip_slice)) // 2 + (window - len(clip_slice)) % 2,
                                                       axis=0)
                    # right slices
...
def get_motion_data(global_positions, global_rotations):
    ...
    forward = filters.gaussian_filter1d(np.cross(across, np.array([[0, 1, 0]])), direction_filterwidth, axis=0,
                                        mode='nearest')
